...
export class Deck {
  ...
  public getCardCount(
    ...
    let result: number = 0;
    if (
      cardListType == CardListType.NewCard || cardListType == CardListType.All
    ) {
      result += this.newFlashcards.length;
    }
    if (
      cardListType == CardListType.DueCard || cardListType == CardListType.All
    ) {
