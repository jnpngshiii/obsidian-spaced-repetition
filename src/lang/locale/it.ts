...
export default {
  ...
  STATUS_BAR:
    "Da rivedere: ${dueNotesCount} scheda/e, ${dueFlashcardsCount} schede in ritardo",
  SYNC_TIME_TAKEN: "Sincronizzato in ${t}ms",
  NOTE_IN_IGNORED_FOLDER:
    "La nota è salvata in una cartella ignorata (rivedi le impostazioni).",
  PLEASE_TAG_NOTE:
  ...
  CHECK_WIKI:
    'Per maggiori informazioni, rivolgersi alla <a href="${wiki_url}">wiki</a>.',
  ...
  RANDOMIZE_CARD_ORDER:
    "Rendere casuale l'ordine delle schede durante la revisione?",
  ...
  REVIEW_CARD_ORDER_NEW_FIRST_SEQUENTIAL:
    "Sequenzialmente dentro il mazzo (prima schede nuove)",
  ...
  REVIEW_CARD_ORDER_NEW_FIRST_RANDOM:
    "A caso dentro il mazzo (prima schede nuove)",
  REVIEW_CARD_ORDER_DUE_FIRST_RANDOM:
    "A caso dentro il mazzo (prima schede in ritardo)",
  REVIEW_CARD_ORDER_RANDOM_DECK_AND_CARD: "Scheda a caso da mazzo a caso",
  REVIEW_DECK_ORDER:
    "L'ordine in cui i mazzi vengono mostrati durante la revisione",
  ...
  CONVERT_HIGHLIGHTS_TO_CLOZES:
    "Convertire ==testo evidenziato== in spazi da riempire?",
  CONVERT_BOLD_TEXT_TO_CLOZES:
    "Convertire **testo in grassetto** in spazi da riempire",
  CONVERT_CURLY_BRACKETS_TO_CLOZES:
    "Convertire {{parentesi graffe}} in spazi da riempire?",
  ...
  INLINE_REVERSED_CARDS_SEPARATOR:
    "Separatore per schede all'incontrario sulla stessa riga",
  MULTILINE_CARDS_SEPARATOR: "Separatore per schede su più righe",
  MULTILINE_REVERSED_CARDS_SEPARATOR:
    "Separatore per schede all'incontrario su più righe",
  NOTES: "Note",
  ...
  MAX_N_DAYS_REVIEW_QUEUE:
    "Numero di giorni massimi da visualizzare nel pannello di destra",
  ...
  LAPSE_INTERVAL_CHANGE:
    "L'intervallo cambierà segnando una scheda / nota come difficile",
  LAPSE_INTERVAL_CHANGE_DESC:
    "Intervallo nuovo = intervallo vecchio * cambio intervallo / 100.",
  ...
  MAX_INTERVAL_MIN_WARNING:
    "L'intervallo massimo deve essere di almeno 1 giorno.",
