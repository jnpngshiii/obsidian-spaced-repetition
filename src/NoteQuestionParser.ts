...
import { ParsedQuestionInfo, parseEx } from "./parser";
...
export class NoteQuestionParser {
  ...
  async createQuestionList(
    ...
    if (hasTopicPaths) {
      ...
      [this.frontmatterTopicPathList, this.contentTopicPathInfo] = this
        .analyseTagCacheList(tagCompleteList);
  ...
  private doCreateQuestionList(
    ...
    for (const parsedQuestionInfo of parsedQuestionInfoList) {
      ...
      let cardScheduleInfoList: CardScheduleInfo[] = NoteCardScheduleParser
        .createCardScheduleInfoList(question.questionText.original);
  ...
  private analyseTagCacheList(
    ...
    const filteredTagCacheList: TagCache[] = tagCacheList.filter(
      (item) =>
        ...
        this.questionList.every(
          (q) =>
            !q.parsedQuestionInfo.isQuestionLineNum(item.position.start.line),
        ),
    );
    ...
    if (filteredTagCacheList.length > 0) {
      ...
      if (frontmatter) {
        ...
        if (frontmatterTagCacheList.length > 0) {
          frontmatterTopicPathList = this.createTopicPathList(
    ...
    //
    const contentStartLineNum: number = frontmatterLineCount > 0
    ...
    for (const t of contentTagCacheList) {
      if (list.length != 0) {
        ...
        if (startLineNum != t.position.start.line) {
          contentTopicPathList.push(
            this.createTopicPathList(list, startLineNum),
          );
          list = [] as TagCache[];
        }
      }
