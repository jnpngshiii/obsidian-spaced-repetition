...
export class TopicPath {
  ...
  static getTopicPathFromCardText(cardText: string): TopicPath {
    const path = cardText.trimStart().match(OBSIDIAN_TAG_AT_STARTOFLINE_REGEX)
      ?.slice(-1)[0];
  ...
  static getFolderPathFromFilename(
    noteFile: ISRFile,
    settings: SRSettings,
  ): TopicPath {
...
export class TopicPathList {
  ...
  static fromPsv(str: string, lineNum: number): TopicPathList {
    const result: TopicPathList = TopicPathList.convertTagListToTopicPathList(
      str.split("|"),
    );
