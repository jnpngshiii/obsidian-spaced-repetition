...
import {
  globalRandomNumberProvider,
  WeightedRandomNumber,
} from "./util/RandomNumberProvider";
...
class SingleDeckIterator {
  ...
  private nextRandomCard(): void {
    ...
    if (newCount + dueCount > 0) {
      ...
      const [cardListType, index] = this.weightedRandomNumber.getRandomValues(
        weights,
      );
      this.setCardListType(cardListType, index);
    } else {
  ...
  private static getCardListTypeForIterator(
    iteratorOrder: IIteratorOrder,
  ): CardListType | null {
...
export class DeckTreeIterator implements IDeckTreeIterator {
  ...
  setIteratorTopicPath(topicPath: TopicPath): void {
    ...
    this.deckArray = DeckTreeIterator.filterForDecksWithCards(
      iteratorDeck.toDeckArray(),
    );
  ...
  private chooseNextDeck(firstTime: boolean): void {
    switch (this.iteratorOrder.deckOrder) {
      ...
      case DeckOrder.PrevDeckComplete_Random: {
        ...
        if (hasDeck) {
          const [deckIdx, _] = this.weightedRandomNumber.getRandomValues(
            weights,
          );
          this.deckIdx = deckIdx;
        } else {
  ...
  private nextCard_EveryCardRandomDeck(): boolean {
    ...
    const weights: Record<number, number> = {};
    for (let i = 0; i < this.deckArray.length; i++) {
      const cardCount: number = this.deckArray[i].getCardCount(
        CardListType.All,
        false,
      );
      if (cardCount) {
    ...
    const [deckIdx, cardIdx] = this.weightedRandomNumber.getRandomValues(
      weights,
    );
