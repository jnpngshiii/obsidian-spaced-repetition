...
export class QuestionText {
  ...
  static create(original: string, settings: SRSettings): QuestionText {
    const [topicPathWithWs, actualQuestion, blockId] = this.splitText(
      original,
      settings,
    );
  ...
  static splitText(
    original: string,
    settings: SRSettings,
  ): [TopicPathWithWs, string, string] {
    const originalWithoutSR = NoteCardScheduleParser.removeCardScheduleInfo(
      original,
    );
    ...
    if (topicPath?.hasPath) {
      ...
      const cardText3: string = cardText2.replaceAll(
        OBSIDIAN_TAG_AT_STARTOFLINE_REGEX,
        "",
      );
      ...
      if (!settings.convertFoldersToDecks) {
        topicPathWithWs = new TopicPathWithWs(
          topicPath,
          preTopicPathWs,
          postTopicPathWs,
        );
...
export class Question {
  ...
  updateQuestionText(noteText: string, settings: SRSettings): string {
    ...
    if (newText) {
      ...
      console.error(
        `updateQuestionText: Text not found: ${
  ...
  static Create(
    ...
    const hasEditLaterTag = parsedQuestionInfo.text.includes(
    ...
    if (questionText.topicPathWithWs) {
      topicPathList = new TopicPathList([
