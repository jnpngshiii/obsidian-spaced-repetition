...
export default class SRPlugin extends Plugin {
  ...
  async onload(): Promise<void> {
    ...
    this.addCommand({
      ...
      callback: async () => {
        const openFile: TFile | null = this.app.workspace.getActiveFile();
        if (openFile && openFile.extension === "md") {
          this.openFlashcardModalForSingleNote(
            openFile,
            FlashcardReviewMode.Review,
          );
        }
  ...
  onunload(): void {
    this.app.workspace.getLeavesOfType(REVIEW_QUEUE_VIEW_TYPE).forEach((leaf) =>
      leaf.detach()
    );
  }
  ...
  private openFlashcardModal(
    ...
    const reviewSequencer: IFlashcardReviewSequencer =
  ...
  private static createDeckTreeIterator(
    ...
    let cardOrder: CardOrder =
      CardOrder[settings.flashcardCardOrder as keyof typeof CardOrder];
    if (cardOrder === undefined) cardOrder = CardOrder.DueFirstSequential;
    let deckOrder: DeckOrder =
      DeckOrder[settings.flashcardDeckOrder as keyof typeof DeckOrder];
    if (deckOrder === undefined) {
    ...
    for (const noteFile of notes) {
      ...
      for (const tagToReview of this.data.settings.tagsToReview) {
        if (
          ...
          if (
            !Object.prototype.hasOwnProperty.call(this.reviewDecks, tagToReview)
          ) {
  ...
  async saveReviewResponse(
    ...
    if (
      ...
      for (const statObj of this.incomingLinks[note.path] || []) {
        ...
        if (ease) {
          linkTotal += statObj.linkCount * this.pageranks[statObj.sourcePath] *
            ease;
      ...
      for (const linkedFilePath in outgoingLinks) {
        ...
        if (ease) {
          linkTotal += outgoingLinks[linkedFilePath] *
            this.pageranks[linkedFilePath] * ease;
          linkPGTotal += this.pageranks[linkedFilePath] *
            outgoingLinks[linkedFilePath];
          totalLinkCount += outgoingLinks[linkedFilePath];
        }
      }

      const linkContribution: number = this.data.settings.maxLinkFactor *
        Math.min(1.0, Math.log(totalLinkCount + 0.5) / Math.log(64));
      ease = (1.0 - linkContribution) * this.data.settings.baseEase +
        (totalLinkCount > 0
    ...
    Object.values(this.reviewDecks).forEach((reviewDeck: ReviewDeck) => {
      ...
      if (!wasDueInDeck) {
        reviewDeck.newNotes.splice(
          reviewDeck.newNotes.findIndex((newNote: TFile) =>
            newNote.path === note.path
          ),
          1,
        );
